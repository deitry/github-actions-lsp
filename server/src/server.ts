...
connection.onDidChangeConfiguration(change => {
	if (hasConfigurationCapability) {
		...
		globalSettings = <ExampleSettings>(
			(change.settings.githubActionsLsp || defaultSettings)
		);
	}
...
function CheckOutputs(text: string, settings: ExampleSettings, textDocument: TextDocument, diagnostics: Diagnostic[]) {
	...
	while ((m = pattern.exec(text)) && problems < settings.maxNumberOfProblems) {
		...
		const diagnostic: Diagnostic = {
			...
			range: {
				start: textDocument.positionAt(matchEnd - 'output'.length),
				end: textDocument.positionAt(matchEnd)
			},
			message: `Possibly a mistake in '${m[0]}'. Did you mean .outputs.?`,
			source: 'ex'
		};
